...
int main() {
    ...
    hour = current_time.tm_min; // direct access
    minute = time_ptr->tm_min;  // access via ptr
    second = *((int *) time_ptr);   // hacky ptr access
    ...
    for(i = 0; i < 3; i++) {
        ...
        int_ptr++; // adding 1 to int_ptr add 4 to the address
    }              // since an int is 4 bytes long
