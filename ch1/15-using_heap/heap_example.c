...
if (argc < 2) // if there are'nt command-line arguments,
	mem_size = 50;
...
printf("\t[+] allocating 12 bytes of memory on the heap ofr int_ptr\n");
...
*int_ptr = 31337; // put value 31337 where int_ptr is pointing.
printf("int_ptr (%p) --> %d\n", int_ptr, *int_ptr);

printf("\t[-] freeing char_ptr's heap memory...\n");
free(char_ptr); // free heap memory

printf("\t[+] allocating another 15 byres for char_ptr\n");
...
free(int_ptr);// free heap memory
printf("\t[-] freeing char_ptr's heap memory...\n");
free(char_ptr); // freeing the other bloc of heap memory
return (0);
}
